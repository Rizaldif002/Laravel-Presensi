    ...
    <div class="px-5 pt-5" x-data="{ modalFilter: false }">
        ...
        <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div class="flex flex-wrap items-center gap-3">
                ...
                @if(request()->hasAny(['tahun_ajaran_id','dosen_id','mata_kuliah_id']))
                    <a href="{{ route('admin.riwayat.index', array_filter(['per_page' => request('per_page')])) }}"
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-blue-50/50 transition-colors">
                        ...
                        <td class="px-4 py-4 text-center">
                            {{-- PERBAIKAN: Memanggil properti total_peserta sesuai dengan alias di Controller --}}
                        ...
                        <td class="px-4 py-4 text-center">
                            {{-- Tombol Detail Presensi --}}
                            <a href="{{ route('admin.riwayat.show', $k->id) }}"
                               class="inline-flex items-center justify-center w-9 h-9 bg-blue-100 text-blue-600 rounded-md hover:bg-blue-600 hover:text-white transition-colors shadow-sm mx-auto" 
                    ...
                    @empty
                    <tr>
                        <td colspan="8" class="px-4 py-16 text-center text-sm font-medium text-gray-400 italic">Belum ada data riwayat presensi yang selesai.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div x-show="modalFilter" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" style="display:none">
            <div @click.outside="modalFilter = false" class="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
                ...
                <form action="{{ route('admin.riwayat.index') }}" method="GET" class="space-y-4">
                    ...
                    <div class="flex gap-3 pt-2">
                        <a href="{{ route('admin.riwayat.index') }}" class="flex-1 text-center py-2 px-4 border border-gray-300 rounded-lg text-sm font-bold text-gray-700 hover:bg-gray-100 transition-all">Reset</a>
