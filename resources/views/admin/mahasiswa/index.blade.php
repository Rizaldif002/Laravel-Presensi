                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    @forelse ($mahasiswas as $index => $mhs)
                        <tr class="odd:bg-white even:bg-gray-50 hover:bg-blue-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $mahasiswas->firstItem() + $index }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <button type="button" onclick="document.getElementById('modalEdit{{ $mhs->id }}').classList.remove('hidden')"
                                    ...
                                    <form action="{{ route('admin.mahasiswa.destroy', $mhs->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data mahasiswa ini?');" class="m-0">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
